...
- `remediation components [path]` — compiles the detected tokens **and** the components found in the codebase into a token-constrained library. Component-agnostic: every component found is re-emitted as its own constrained component (no archetype catalog), near-duplicates are merged into one canonical component, and each component's detected style values become its own token-typed props + defaults. Unmapped values stay literal in a `LOOK` style block and are reported. Reuses the primitives output (`tokens.ts`/`primitives.tsx`). `--output <dir>` (default `components/`).
